...
function CustomAlert() {
  this.alert = function (message, title) {
    ...
    dialogContent.innerHTML =
      '<div id="alertHeader"></div><div id="alertBody"></div><div id="alertFooter"></div>';
    ...
    enviarButton2.addEventListener("click", function (event) {
      ...
      if (inputImagem2.files && inputImagem2.files[0]) {
        ...
        reader.onload = function (e) {
          ...
          var img = new Image();
          img.src = imagemBase642;
          img.onload = function () {
            var canvas = document.createElement("canvas");
            var ctx = canvas.getContext("2d");

            // Defina o tamanho desejado do recorte
            var cropWidth = 550;
            var cropHeight = 550;

            // Defina as coordenadas iniciais do recorte
            var cropX = (img.width - cropWidth) / 2;
            var cropY = (img.height - cropHeight) / 2;

            // Ajuste o tamanho do canvas para o tamanho do recorte
            ...
            // Obtenha a imagem recortada como base64
            var croppedImageBase64 = canvas.toDataURL();

            localStorage.setItem("imagempessoa", croppedImageBase64);
